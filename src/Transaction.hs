...
layout :: (Entity Book -> Entity Transaction -> [Entity TransactionAccount] -> [AccountTree] -> Widget) -> BookId -> TransactionId -> Handler Html
layout f bookId transactionId = flip Book.layout bookId $ \bookE accountTree -> do
    ...
    f bookE (Entity transactionId t) ts accountTree
